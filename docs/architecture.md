...
vendor SDK. Invokr is the first host, but nothing in this repository is specific
to it: the host-facing surface is either a plain Go interface or a JSON-RPC
...
this repository with a compatibility promise. The Go API is pre-1.0 and has
broken twice (ADR 0004, ADR 0007); `llm.v1` is additive.
...
- [`adr/`](adr/) — the decision record, including why the seam exists (0001),
  ...
  (0005), the Provider Process threat model (0006), and the rename (0007).
