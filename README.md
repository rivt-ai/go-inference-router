...
Invokr is the first host, but nothing here is Invokr-specific: the host-facing
...
Pre-1.0. Go source compatibility intentionally broke when
`inference-core/inference` became `invokr-llm`, and again when `invokr-llm`
became `go-inference-router`. The additive `llm.v1` process protocol is the
compatibility commitment — pin to it if you want stability today.
