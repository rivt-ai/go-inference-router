...
Invokr's LLM access lives in `clients/`, built on `github.com/openai/openai-go`.
The SDK's types are not confined to that package: `openai.ChatCompletionNewParams`
and the message unions flow through the runner, so every component that touches
a turn is coupled to one vendor's wire format. Invokr's own extension strategy
records this as the reason a provider plugin interface was deferred — the
coupling has to be paid down first.

Two further pressures:

- Invokr's `goclocbudget` caps implementation Go at a fixed size and fails CI on
  size alone. Growing multi-provider support inside Invokr competes with feature
  work for the same budget.
- The backend seam is useful beyond Invokr, and it changes for different reasons
...
- Invokr can adopt this behind its existing `clients.LLMProvider` interface,
  translating at one boundary, and delete its SDK dependency once the runner
